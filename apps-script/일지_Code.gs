...
function doGet() {
  const lines = [
    ...
    "배포 확인 코드 : " + DEPLOY_TAG,
    "",
    "정상 동작 중입니다. 학생들이 제출할 수 있습니다.",
    "",
    "이 화면이 안 보이거나 확인 코드가 다르면, 이 URL은 예전 코드를 돌리고",
    "있는 것입니다. [배포] > [배포 관리] > 연필(수정) > 버전 '새 버전' > [배포]",
    "로 갱신하세요. ([새 배포]를 누르면 URL이 새로 생기니 주의하세요)"
  ];
